...
このファイルはリポジトリで作業する際の Codex へのガイダンスを提供します。
...
`.Codex/hooks/session-start.sh` にセッション開始フックが設定されています：
- リモート環境（Codex on the web）でのみ実行
- git のユーザー情報を設定
- `pnpm install` で依存関係をインストール（非同期）

フックは `.Codex/settings.json` に登録されています。

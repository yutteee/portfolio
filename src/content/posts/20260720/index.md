...
768px を超えた直後は、PC 用の文字サイズがそのまま当たります。ところが 768px 付近はまだ画面としては小さめなので、PC 用の大きな文字を当てると、今度は文字が大きすぎるように感じてしまいます。ここで「レイアウトが最適化された状態から外れてしまう」という問題が起きます。
...
文字はひとつのサイズだけではなく、本文・見出しなどの階層があります。そこで**タイプスケール（type scale）**として階層を定義し、その各段のサイズは**モジュラースケール（modular scale）＝一定の比率**から生成します。たとえば比率 1.2 なら、1 段上がるごとにサイズが 1.2 倍になっていきます。スペースも同じ考え方で、小さい画面・大きい画面それぞれの余白を決めれば間は補間され、決まったスペースはグリッド（カラムや溝）の設計にもつながっていきます。

...
export const HeaderPresenter: React.FC<HeaderPresenterProps> = ({ menuOpen, onOpen, onClose, closeBtnRef, isDark, onThemeToggle, darkBtnRef, lightBtnRef, currentPage, spMenuRef, hamburgerRef }) => {
  return (
    <header className={styles.header}>
      <a href="/" className={styles.logo} aria-label="トップ">Yutteee</a>
      <div className={styles.nav}>
        <nav className={styles.navLinks} aria-label="メインナビゲーション">
          <a href="/about/" className={currentPage === "私について" ? styles.current : undefined}>私について</a>
          <a href="/products/" className={currentPage === "プロダクト" ? styles.current : undefined}>プロダクト</a>
          <a href="/posts/" className={currentPage === "記事" ? styles.current : undefined}>記事</a>
        </nav>
      ...
      >
        <div className={styles.close}>
          <IconButton label="メニューを閉じる" icon="FiX" id="close" handleClick={onClose} ref={closeBtnRef} />
        </div>
